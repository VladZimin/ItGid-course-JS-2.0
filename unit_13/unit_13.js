...
function f16() {
    let out = ''
    for (let key in a16) {
        for (let nesKey in a16[key]) {
            if (nesKey === 'name') {
                out += a16[key][nesKey] + ' '
            }
        }
    }
    out16.textContent = out
}
...
function f19() {
    let val = i19.value.toLowerCase()
    let out = ''
    for (let key in a19) {
        let arr = []
        for (let i = 0; i < a19[key].length; i++) {
            arr.push(a19[key][i].toLowerCase())
        }
        if (arr.includes(val)) {
            out = key
        }
