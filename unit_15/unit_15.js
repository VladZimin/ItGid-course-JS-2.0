...
const f4 = () => {
    out4.textContent = `${s4.has(i4.value)}`
}
...
const f7 = () => {
    let val = i7.value.toLowerCase()
    let s7 = new Set(val)
    let str = Array.from(s7).join('')
    if (val === str && val.length > 6) {
        out7.textContent = 1
    } else {
        out7.textContent = 0
    }
